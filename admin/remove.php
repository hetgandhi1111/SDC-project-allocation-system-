...
if(isset($_GET['pno'])){
    ...
    <div class='bg-light'>
            <h3 class='text-center p-3'><p>Are you sure you want to remove add  name: <span style='color: #ff6600'> $name </span>, pno:<span style='color: #ff6600'> $pno_s </span>?</p></h3>
        </div>
...
?>
    <form action="" method="post">
            ...
            <!-- Submit -->
            <div class="form-outline mb-4 w-50 m-auto">
                <input type="submit" name="insert_data" class='btn btn-secondary m-2' style='border-radius:50px' value="Remove Data">
...
if(isset($_POST['insert_data'])){
    ...
    $delete_query="delete from `selected` where `pno`='$pno_s'";
    $result_query=mysqli_query($con,$delete_query);
    if($result_query){
        echo"
        <script>
        alert('data removed successfully for pno: $pno_s');
        window.location.href='index.php?view_all1';
        </script>
        ";

$set="update `requests` set `accepted`='0' where `pno`='$pno_s'";
$set_result=mysqli_query($con,$set);
if(!$set_result){

    echo"<script>alert('error in setting accepted to 0')</script>";
